            ...
            <div class="category-list">
                @foreach ($categories as $category)
                    <div class="category-item">
                        <div
                            class="category-header d-flex align-items-center justify-content-between p-2 {{ $loop->index % 2 == 0 ? 'bg-light' : 'bg-white' }}">
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <span class="category-name"
                                    @if (collect($category->children)->isNotEmpty()) data-bs-toggle="collapse"
                                ...
                                <div class="subcategory-list ps-4">
                                    @foreach ($category->children as $child)
                                        <div class="subcategory-item">
                                            <div
                                                class="subcategory-header d-flex align-items-center justify-content-between {{ $loop->index % 2 == 0 ? 'bg-light' : 'bg-white' }}">
                                                <div class="d-flex align-items-center gap-2">
                                                    ...
                                                    @endif
                                                    <span
                                                        @if (collect($child->children)->isNotEmpty()) data-bs-toggle="collapse"
                                                    data-bs-target="#subcollapse-{{ $loop->parent->index }}-{{ $loop->index }}" @endif>
                                            ...
                                            @if (collect($child->children)->isNotEmpty())
                                                <div class="collapse ps-4"
                                                    id="subcollapse-{{ $loop->parent->index }}-{{ $loop->index }}">
                                                    <div class="sub-subcategory-list ps-4">
                                                        @foreach ($child->children as $subChild)
                                                            <div class="sub-subcategory-item">
                                                                <div
                                                                    class="d-flex align-items-center justify-content-between {{ $loop->index % 2 == 0 ? 'bg-light' : 'bg-white' }}">
                                                                    <div class="d-flex align-items-center gap-2">
                                                                        ...
                                                                        @endif
                                                                        <span
                                                                            @if (collect($subChild->children)->isNotEmpty()) data-bs-toggle="collapse"
                                                                        data-bs-target="#subsubcollapse-{{ $loop->parent->parent->index }}-{{ $loop->parent->index }}-{{ $loop->index }}" @endif>
                                                                            {{ $subChild->family_category_name }}
                                                                        </span>
                                                                        <i class="fa-regular fa-pen-to-square"></i>
                                                                    </div>
                                                                    <div
                                                                        class="category-actions d-flex align-items-center gap-2">
                                                                        <input class="form-check-input" type="checkbox">
                                                                        <button class="btn btn-sm btn-light"><i
